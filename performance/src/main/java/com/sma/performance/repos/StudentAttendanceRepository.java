...
public interface StudentAttendanceRepository extends JpaRepository<StudentAttendance, Integer> {
	...
	@Query(value="select sa.id, sa.created_at, sa.date, sa.teacher_id  from student_attendance sa where sa.date=?1 order by created_at desc limit 1 ", nativeQuery=true)
